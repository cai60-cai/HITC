    ...
    <div id="primary" class="content-area">
      <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        <el-form-item label="反馈类型" prop="feedbackType">
          <el-radio-group v-model="ruleForm.feedbackType" v-if="$device.windows">
            <el-radio :label="item" :key="index" v-for="(item, index) in feedbackType">{{item}}</el-radio>
          </el-radio-group>
          <el-select v-model="ruleForm.feedbackType" v-else>
            <el-option :label="item" :value="item" v-for="item in feedbackType" :key="item"/>
          </el-select>
        </el-form-item>
        <el-form-item label="反馈图片" prop="avatar">
        ...
        <el-form-item label="接收邮箱" prop="feedbackEmail">
          <el-input
              placeholder="请输入接收邮箱,不填默认为资料邮箱"
              show-word-limit
              autocomplete="off"
              maxlength="50"
              v-model="ruleForm.feedbackEmail">
          </el-input>
        </el-form-item>
...
import {questionList} from "@/api/main";
import {addFeedback} from "@/api/feedback";
export default {
  name: 'Fallback',
  components: {FallbackSide},
  data() {
    var validatefeedbackType = (rule, value, callback) => {
      if (!value) {
        return callback(new Error('反馈类型为必选项'));
      }
      callback()
    };
    var validatefeedbackContent = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('反馈内容为必填项'));
      } else {
        if (value.length > 1000) {
          callback(new Error('反馈内容不能大于1000字符'));
        }
        callback();
      }
    };
    ...
    return {
      ...
      rules: {
        feedbackType: [
          {validator: validatefeedbackType, trigger: 'blur'}
        ],
        feedbackContent: [
          {validator: validatefeedbackContent, trigger: 'blur'}
        ],
        feedbackEmail: [
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        this.ruleForm.feedbackImages = arr
        if (valid) {
